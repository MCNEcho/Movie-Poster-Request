...
function buildMainBoard_() {
  ...
  posterIds.forEach(pid => {
    ...
    list.forEach(r => {
      const note = String(r[COLS.REQUESTS.NOTES - 1] || '').trim();
      out.push([String(r[COLS.REQUESTS.EMP_NAME - 1]), r[COLS.REQUESTS.REQ_TS - 1], note]);
    });
  ...
  main.getRange(1, 1, out.length, 3).setValues(out);

  // Clear all rows below the new data to remove orphaned entries
  const maxRows = main.getMaxRows();
  if (out.length + 1 <= maxRows) {
    main.getRange(out.length + 1, 1, maxRows - out.length, 3).clearContent();
  }

  const used = main.getRange(1, 1, out.length, 3);
...
function buildEmployeesBoard_() {
  ...
  names.forEach(name => {
    ...
    list.forEach(r => {
      const pid = String(r[COLS.REQUESTS.POSTER_ID - 1]);
      const note = String(r[COLS.REQUESTS.NOTES - 1] || '').trim();
      out.push([idToLabel[pid] || String(r[COLS.REQUESTS.LABEL_AT_REQ - 1]) || pid, r[COLS.REQUESTS.REQ_TS - 1], note]);
    });
  ...
  empSheet.getRange(1, 1, out.length, 3).setValues(out);

  // Clear all rows below the new data to remove orphaned entries
  const maxRows = empSheet.getMaxRows();
  if (out.length + 1 <= maxRows) {
    empSheet.getRange(out.length + 1, 1, maxRows - out.length, 3).clearContent();
  }

  const used = empSheet.getRange(1, 1, out.length, 3);
  used.setBackground('#ffffff');
  used.setFontWeight('normal');
  used.setHorizontalAlignment('left');
  used.setFontColor('#000000');
